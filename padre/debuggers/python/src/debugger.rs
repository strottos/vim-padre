//! Python debugger
//!
//! The main PDB Debugger entry point. Handles listening for instructions and
//! communicating through an `PythonProcess` object.

use std::process::exit;
use std::time::Instant;

use super::process::{Message, PythonProcess};
...
impl PythonDebugger {
    pub fn new(
        ...
        let process = PythonProcess::new(debugger_cmd, run_cmd, notifier_tx.clone());
    ...
    /// Perform any initial setup including starting PDB and setting up the stdio analyser stuff
    /// - startup pdb and setup the stdio analyser
    /// - perform initial setup so we can analyse PDB properly
    ...
    fn run(&mut self, timeout: Instant, tx_done: oneshot::Sender<Result<serde_json::Value>>) {
        ...
        self.process.send_msg(Message::Restart, Some(tx));
    }
